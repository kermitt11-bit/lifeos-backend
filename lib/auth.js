...
import { MemClient, seedDemoUser } from './memstore.js';
...
function demoUserIdFromReq(req) {
  const raw =
    req.headers['x-demo-user'] ||
    req.cookies?.demo_user ||
    req.query.demo_user;
  if (!raw) return null;
  // Normalize to a deterministic-looking id; we don't actually need a UUID,
  // but we use it as a stable key throughout the in-memory store.
  return String(raw).slice(0, 64);
}

export function requireAuth(req, _res, next) {
  // Demo header is always accepted: lets the bundled SPA work even on a
  // deploy that has SUPABASE_URL set for production clients.
  const demoId = demoUserIdFromReq(req);
  if (demoId || isDemoMode()) {
    const id = demoId || 'demo-anon';
    seedDemoUser(id);
    req.user = { id, email: `${id}@demo.local` };
    req.db = new MemClient(id);
    return next();
  }
